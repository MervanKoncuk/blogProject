...
def userLogin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, "Kullanıcı adı veya şifre hattalı!")
    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            if 'takip' in request.POST:
                ...
                if hesabim in profil.followers.all():
                    profil.followers.remove(hesabim)
                    hesabim.follow.remove(profil) # profil'i takip eden kullanıcının follow'undan kaldır
                    messages.success(request, f'{profil.user.username} kullanıcısını takip etmeyi bıraktınız.')
                    return redirect('profile', slug=slug)
                else:
                    profil.followers.add(hesabim)
                    hesabim.follow.add(profil) # profil'i takip eden kullanıcının follow'una ekle
